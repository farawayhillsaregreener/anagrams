...
This script is written to find anagrams from words from a known dictionary list. 
For example, if the word post is in the list, then this script would return
...
combinedList=sortedLetterList=None 

  
#####    FORMAT  and  OUTPUT RESULTS
#unique List of anagrams
lesserGramList=[]
for grams in gramList:
    if grams not in lesserGramList:
        lesserGramList.append(grams)

#formatting, adding commas, and end of line returns          
for i in range(0,len(lesserGramList)):
    dup=lesserGramList[i][:]
    lesserGramList[i]=[y+', ' for y in lesserGramList[i][:-1]]
    lesserGramList[i].append(dup[len(dup)-1]+'\n')
    lesserGramList[i]=[''.join(lesserGramList[i])]

#write out the reuslts out to the previously opened file
for lines in lesserGramList:
    a.writelines(lines)

a.close()

lesserGramList=gramList=a=lines=i=x=None
